...
(function () {
  ...
  class LiveVoiceManager {
    ...
    startOrbRenderer() {
      ...
      const particles = [];
      const particleColors = ['#38bdf8', '#60a5fa', '#93c5fd', '#2563eb', '#bfdbfe'];
      for (let p = 0; p < 36; p++) {
        particles.push({
      ...
      const render = () => {
        ...
        centerGlow.addColorStop(0, 'rgba(56, 189, 248, 0.45)');
        centerGlow.addColorStop(0.45, 'rgba(37, 99, 235, 0.25)');
        centerGlow.addColorStop(0.8, 'rgba(14, 45, 92, 0.12)');
        ...
        if (this.sEmblemEl) {
          ...
          this.sEmblemEl.style.filter = `drop-shadow(0 0 ${sGlowBlur.toFixed(1)}px rgba(56, 189, 248, 0.75)) drop-shadow(0 0 ${(sGlowBlur * 1.6).toFixed(1)}px rgba(37, 99, 235, 0.45))`;
        }
    ...
    updateTelemetryAnalysis(userText) {
      ...
      const selfHarmPhrases = [
        'self harm', 'selfharm', 'self-harm', 'self injury', 'selfinjury', 'self-injury', 'self-inflicted', 'self inflict',
        'self mutilation', 'self mutilate',
        'hurt myself', 'hurting myself', 'harm myself', 'harming myself', 'cut myself', 'cutting myself',
        'burn myself', 'burning myself', 'bleed myself', 'bleeding myself', 'injure myself', 'injuring myself',
        'punish myself', 'punishing myself', 'scratch myself', 'scratching myself', 'hit myself', 'hitting myself',
        'pain myself', 'inflict pain', 'inflicting pain',
        'blade', 'razor', 'slit',
        'cut my wrist', 'cut my arm', 'cut my hand', 'cut my leg', 'cut my skin', 'cut my body', 'cut my neck',
        'cutting my wrist', 'cutting my arm', 'cutting my hand', 'cutting my leg',
        'slit my wrist', 'slit my arm', 'slit my neck', 'slit my throat',
        'hurt self', 'harm self', 'cut self', 'cut hand', 'cut wrist', 'cut arm', 'cut body',
        // Hindi / Hinglish
        'khud ko chot', 'khud ko nuksan', 'khud ko takleef', 'khud ko dard', 'khud ko harm',
        'apne aap ko chot', 'apne ko chot', 'apne aap ko nuksan', 'apne ko nuksan',
        'apne hath kaat', 'apne haath kaat', 'haath kaat', 'haath kat', 'hath kaat', 'hath kat',
        'nus kaat', 'nas kaat', 'nas kat', 'nus kat', 'nasein kaat', 'nasen kaat',
        'chaku se hath', 'chaku se haath', 'chaku se cut', 'blade se', 'blade maarna', 'blade marna', 'blade se kaat',
        'khud ko jalana', 'apne ko jalaya', 'apne aap ko jalana',
        'khud ko marunga', 'apne aap ko marunga', 'khud ko zakhmi', 'apne shareer ko chot', 'khud ko aghat',
        'shareer ko chot', 'zakhmi karna',
        // Devanagari Hindi Script
        'खुद को चोट', 'अपने आप को चोट', 'हाथ काट', 'हाथ काटना', 'नस काट', 'नस काटना', 'ब्लेड', 'खुद को नुकसान',
        'खुद को दर्द', 'खुद को तकलीफ', 'चाकू से काट', 'खुद को जलाना', 'अपने हाथ पर कट', 'जख्मी करना',
        'खुद को मारना', 'खुद को मारूंगा', 'खुद को नुकसान पहुँचाना', 'खुद को चोट पहुंचा',
        // Bengali
        'nije ke aghat', 'nijeke aghat', 'nije kosto dewa', 'haat kata', 'rokto ber kora', 'nijeke kosto',
        'হাত কাটা', 'নিজেকে আঘাত', 'রক্ত বের করা', 'নিজেকে কষ্ট'
      ];

      // Threat / Violence / Intimidation triggers (English, Hindi, Hinglish, Bengali)
      const threatPhrases = [
        'threat', 'threaten', 'threats', 'intimidation', 'intimidat', 'blackmail', 'stalk',
        'harass', 'assault', 'weapon', 'police bula',
        'kill you', 'destroy you', 'harm you', 'shoot you', 'hurt you', 'beat you', 'stab you',
        'outside my house', 'outside your house', 'attacking me', 'attacked me', 'breaking into',
        'chaku', 'knife', 'bandook', 'gun', 'pistol',
        'danger to my life', 'threat to my life', 'life in danger',
        'somebody is following', 'someone is following', 'following me', 'chasing me',
        'force me', 'forced me', 'forcing me', 'he will kill me', 'she will kill me',
        'they will kill me', 'someone wants to kill',
        // Hindi / Hinglish
        'dhamki', 'dhamka', 'dhamkana', 'dhamki di', 'dhamki de', 'dhamki de raha', 'dhamki mil rahi',
        'darana', 'dara raha', 'dekh lunga', 'barbaad kar dunga', 'barbad kar dunga',
        'pit dunga', 'maar peet', 'chaku dikha raha', 'piche pada', 'picha kar raha',
        'blackmail kar raha', 'badla lunga', 'mar dunga', 'maar dunga', 'maar dalunga',
        'jaan se maar', 'jaan se marne ki', 'goli maar', 'teri jaan le lunga',
        // Devanagari Hindi Script
        'धमकी', 'धमका', 'डरा रहा', 'मार दूंगा', 'मार डालूंगा', 'जान से मार', 'गोली मार',
        'देख लूँगा', 'बर्बाद कर दूंगा', 'चाकू', 'पिस्तौल', 'बंदूक', 'ब्लैकमेल', 'पीछा कर रहा',
        'धमकी दी', 'जान से मारने की', 'धमकी मिल रही',
        // Bengali
      ...
      const safetyRecoveryPhrases = [
        'im safe', "i'm safe", 'i am safe', 'safe now', 'feeling safe', 'feel safe',
        'feeling ok', 'feeling okay', 'feel ok', 'feel okay', 'feeling good', 'feel good',
        'feeling better', 'feel better', 'much better', 'all good', 'im ok', "i'm ok",
        'i am ok', 'im okay', "i'm okay", 'i am okay', 'calm now', 'better now',
        'out of danger', 'not suicidal', 'dont worry', "don't worry", 'i feel relaxed',
        'safe and okay', 'safe and sound', 'now i am safe', 'now im safe',
        // Hindi / Hinglish
        'achha feel', 'achha feel kar raha', 'achha feel ho raha', 'theek hu', 'mai theek hu',
        'ab theek hu', 'sab theek hai', 'theek lag raha', 'bach gaya', 'sukun mila',
        'sukoon mila', 'tension nahi hai', 'dar nahi lag raha', 'shant hu',
        'theek thaak hu', 'safe hu', 'surakshit hu',
        // Devanagari Hindi Script
        'अच्छा फील', 'अच्छा महसूस', 'मैं ठीक हूँ', 'मैं ठीक हू', 'अब ठीक हूँ', 'सब ठीक है',
        'सुरक्षित हूँ', 'सकून मिला', 'सुकून मिला', 'शांत हूँ',
        'बेहतर महसूस', 'डर नहीं लग रहा', 'अच्छा लग रहा',
        // Bengali
        'bhalo achi', 'ami safe', 'ar kosto nei', 'thik achi', 'shanto achi', 'bhalo lagche'
      ];

      // Helper function to normalize text (handles punctuation, extra whitespace, Unicode)
      const normalizeText = (str) => {
        return (str || '')
          .toLowerCase()
          .replace(/[^a-z0-9\u0900-\u09FF\u0980-\u09FF\s]/g, ' ')
          .replace(/\s+/g, ' ')
          .trim();
      };

      // Resilient matcher that handles word stems, multi-word phrases, and Devanagari/Bengali
      const matchesPhraseList = (phraseList, rawText) => {
        if (!rawText) return false;
        const normTarget = normalizeText(rawText);
        if (!normTarget) return false;
        const targetWords = normTarget.split(' ');

        return phraseList.some(p => {
          const normP = normalizeText(p);
          if (!normP) return false;
          // Multi-word phrase or Non-Latin (Devanagari, Bengali) substring match
          if (normP.includes(' ') || /[\u0900-\u09FF\u0980-\u09FF]/.test(normP)) {
            return normTarget.includes(normP);
          }
          // Single word or root stem match (e.g. 'threat' matches 'threat', 'threats', 'threatening', 'threatened')
          return targetWords.some(w => w === normP || w.startsWith(normP));
        });
      };

      // Check triggers on current utterance
      const isCurrentSuicide = matchesPhraseList(suicidePhrases, lower);
      const isCurrentSelfHarm = matchesPhraseList(selfHarmPhrases, lower);
      const isCurrentThreat = matchesPhraseList(threatPhrases, lower);

      // Check triggers across cumulative context
      const isContextSuicide = matchesPhraseList(suicidePhrases, fullContextLower);
      const isContextSelfHarm = matchesPhraseList(selfHarmPhrases, fullContextLower);
      const isContextThreat = matchesPhraseList(threatPhrases, fullContextLower);

      // Explicit recovery triggered if CURRENT utterance confirms safety and NO new crisis words are in current utterance
      const isSafetyConfirmed = matchesPhraseList(safetyRecoveryPhrases, lower) && 
        !isCurrentSuicide && !isCurrentSelfHarm && !isCurrentThreat;

      if (isSafetyConfirmed) {
        // Explicit reassurance & safety recovery: reset crisis flags to safe / normal
        ...
        // If crisis triggered in current utterance or prior context (when not de-escalated)
        if (isCurrentSuicide || (!this.sessionDetectedSafetyFlags.deescalated && isContextSuicide)) {
          this.sessionDetectedSafetyFlags.suicide = true;
        }
        if (isCurrentSelfHarm || (!this.sessionDetectedSafetyFlags.deescalated && isContextSelfHarm)) {
          this.sessionDetectedSafetyFlags.selfHarm = true;
        }
        if (isCurrentThreat || (!this.sessionDetectedSafetyFlags.deescalated && isContextThreat)) {
          this.sessionDetectedSafetyFlags.threat = true;
        }

        if (this.sessionDetectedSafetyFlags.suicide || this.sessionDetectedSafetyFlags.selfHarm || this.sessionDetectedSafetyFlags.threat) {
          this.sessionDetectedSafetyFlags.deescalated = false;
          this.sessionDetectedSafetyFlags.safetyConfirmed = false;
        }
      }
    ...
    recalculateLiveTelemetry() {
      ...
      const hasCriticalTrigger = this.sessionDetectedSafetyFlags.suicide || this.sessionDetectedSafetyFlags.selfHarm || this.sessionDetectedSafetyFlags.threat;
    ...
    renderTelemetryUI(trendState, trendArrow) {
      ...
      if (this.vdSupportList) {
        ...
        if (flags.suicide || flags.selfHarm) {
          supportBullets.push('<div class="vd-support-bullet" style="color:#f87171;font-weight:700;">Immediate crisis safety protocol recommended</div>');
          supportBullets.push('<div class="vd-support-bullet">Connect with certified 24x7 emergency helpline</div>');
          supportBullets.push('<div class="vd-support-bullet">Provide grounding and safety reassurance</div>');
        }
        if (flags.threat) {
          supportBullets.push('<div class="vd-support-bullet" style="color:#fb923c;font-weight:700;">Active safety alert: ensure caller is in a secure location</div>');
          supportBullets.push('<div class="vd-support-bullet">Provide emergency contact information and crisis escalation</div>');
          if (!flags.suicide && !flags.selfHarm) {
            supportBullets.push('<div class="vd-support-bullet">Maintain supportive, reassuring communication</div>');
          }
        }
        if (!flags.suicide && !flags.selfHarm && !flags.threat) {
          if (this.currentSvi >= 61 || flags.severeDistress) {
            supportBullets.push('<div class="vd-support-bullet">Active compassionate de-escalation</div>');
            supportBullets.push('<div class="vd-support-bullet">Encourage deep calming breaths & grounding</div>');
            supportBullets.push('<div class="vd-support-bullet">Provide emotional validation without rush</div>');
          } else if (this.currentSvi >= 41) {
            supportBullets.push('<div class="vd-support-bullet">Empathetic listening and validation</div>');
            supportBullets.push('<div class="vd-support-bullet">Explore manageable steps to address concerns</div>');
            supportBullets.push('<div class="vd-support-bullet">Provide calming conversational presence</div>');
          } else {
            supportBullets.push('<div class="vd-support-bullet">Continue conversation</div>');
            supportBullets.push('<div class="vd-support-bullet">Provide emotional support and guidance</div>');
          }
        }
        this.vdSupportList.innerHTML = supportBullets.join('');
      }
    }
  }
